...
#Request to retrieve current user information
...
async def read_books(
    ...
    books = db.query(models.Book).filter(models.Book.user_id == current_user.id).offset(skip).limit(limit).all()
    return books

#Request to get books by id
...
# Request to update a book by id
...
async def update_book(book_id: int, book: schemas.BookCreate, db: Session = Depends(database.get_db), current_user: models.User = Depends(auth.get_current_user)):
    db_book = db.query(models.Book).filter(models.Book.id == book_id, models.Book.user_id == current_user.id).first()
    if db_book is None:
        raise HTTPException(status_code=404, detail="Book not found")
